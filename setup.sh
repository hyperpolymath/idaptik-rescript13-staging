...
# setup.sh — Universal setup script for idaptik-rescript13-staging
#
# Detects your shell, platform, and installs prerequisites.
# Then hands off to `just setup` for project-specific configuration.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hyperpolymath/idaptik-rescript13-staging/main/setup.sh | sh
#   # or after cloning:
#   ./setup.sh
#
...
# ── Main ──
main() {
    printf "%s=== idaptik-rescript13-staging Setup ===%s\n\n" "$BOLD" "$RESET"
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/idaptik-rescript13-staging.git"
        info "Then: cd idaptik-rescript13-staging && ./setup.sh"
        exit 1
    fi
